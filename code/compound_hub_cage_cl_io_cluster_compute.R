...
for(tmp_res in tmp_res_set){
  ...
  io_dat_l<-lapply(chr_set,function(chromo){
    ...
    chr_cl_hic_dat<-chr_dat %>% filter(X1 %in% cl_bins & X2 %in% cl_bins) %>% 
      filter(d<=max_d)
    
    tmp_cl_tbl<-tibble(chr=chromo,res=tmp_res,cl=tmp_cage_cl,bins=chr_spec_res$cl_member[tmp_cage_cl]) %>% 
      mutate(starts=map(bins,as.numeric),ends=pmap(list(bins,res),function(bin,res){
        as.numeric(bin) + res_num[res] -1
      })) %>% 
      dplyr::select(-bins) %>% 
      mutate(GRange=pmap(list(chr,starts,ends),function(chr,starts,ends){
