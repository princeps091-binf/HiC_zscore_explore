...
names(chr_res_l)<-chr_set
chromo<-"chr22"
  tmp_res_l<-vector('list',length(res_set))
  names(tmp_res_l)<-res_set
  for(cl_res in res_set){
    ...
    tmp_res_l[[cl_res]]<-chr_bin_dat 
  }
  chr_res_tbl<-do.call(bind_rows,tmp_res_l) 
  ...
  for(cl_res in res_set){
    ...
    tmp_res_l[[cl_res]]<-chr_dat %>% mutate(res=cl_res)
  }
  chr_zscore_tbl<-do.call(bind_rows,tmp_res_l)
