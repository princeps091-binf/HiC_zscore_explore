...
tmp_res_set<-unique(trans_hub_tbl$res)
# Formula to set maximum size capping:
# 20000*1024^2 = 2097152000
options(future.globals.maxSize=20971520000)
for(tmp_res in tmp_res_set){
  hub_chr<-trans_hub_tbl %>% filter(res==tmp_res) %>% distinct(chr) %>% unlist
  ...
  io_dat_l<-lapply(chr_set,function(chromo){
    ...
    hub_chr_dat<-chr_dat %>% filter(X1 %in% unique(unlist(tmp_hub$bins)) & X2 %in% unique(unlist(tmp_hub$bins)))
