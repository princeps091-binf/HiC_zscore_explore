...
for(chromo in chr_set){
  ...
  message(chromo, ": CAGE-bin intersection")
  ...
  save(io_hub_dat,file=paste0("./data/hub_io_zscore/GM12878/zscore_5kb/",chromo,"_io_hub_dat.Rda"))
  
}


io_dat_tbl<-do.call(bind_rows,lapply(list.files("./data/hub_io_zscore/GM12878/zscore_5kb/"),function(dat_file){
  data_tbl_load_fn(paste0("./data/hub_io_zscore/GM12878/zscore_10kb/",dat_file))
}))
io_dat_tbl %>% 
  ggplot(.,aes(zscore,color=hub.io))+
  geom_density()
save(io_dat_tbl,file = "./data/hub_io_zscore/GM12878/GM12878_hub_cage_zscore_io_tbl_5kb.Rda")
