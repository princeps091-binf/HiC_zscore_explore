...
for(tmp_res in tmp_res_set){
  ...
  io_dat_l<-lapply(chr_set,function(chromo){
    ...
    tmp_hub<-tmp_hub %>% mutate(hub.feature.zscore=future_map(bins,function(x){
      chr_dat %>% filter(X1 %in% x & X2 %in% x) %>% dplyr::select(chr,res,X1,X2,zscore)
    }))
  ...
  save(tmp_res_tbl,file = paste0("~/data_transfer/io_hub_out_tbl/GM12878/GM12878_io_zscore_",tmp_res,"_tbl.Rda"))
