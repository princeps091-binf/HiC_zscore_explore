...
for(tmp_res in tmp_res_set){
  ...
  io_dat_l<-lapply(chr_set,function(chromo){
    ...
    tmp_hubs<-top_compound_hub_5kb_tbl %>% 
      filter(res==tmp_res & chr == chromo) %>% 
      dplyr::select(node) %>% 
      unlist
    ...
    # Find the clusters found at considered resolution
    res_cage_cluster<-grep(tmp_res,names(chr_spec_res$cl_member),value=T)
    
    
    plan(multisession, workers = 15)
    # Focus on CAGE-containing clusters
    tmp_cage_cl<-res_cage_cluster[map_lgl(res_cage_cluster,function(x){
      any(chr_cage_bin_dat$bins %in% chr_spec_res$cl_member[[x]])
    })]
    plan(sequential)
    
    # Extract all bins contained in cluster at considered resolution
    cl_bins<-as.numeric(unique(unlist(chr_spec_res$cl_member[tmp_cage_cl])))
    # Use maximum cluster span to further constrain HiC data
    max_d<-max(map_dbl(res_cage_cluster,function(x){
      diff(range(as.numeric(chr_spec_res$cl_member[[x]])))
    }))
    # Subset HiC data to consider bins found within clusters and restriced to maximum cluster span observed
    chr_cl_cage_hic_dat<-chr_cage_hic_dat %>% filter(X1 %in% cl_bins & X2 %in% cl_bins) %>% 
      filter(d<=max_d)
    
    tmp_cl_tbl<-tibble(chr=chromo,res=tmp_res,cl=tmp_cage_cl,bins=chr_spec_res$cl_member[tmp_cage_cl]) %>% 
      mutate(starts=map(bins,as.numeric)) %>% 
      dplyr::select(-bins) 
    
    plan(multisession, workers = 15)
    
    message(chromo," at ",tmp_res,": Subset hub interaction")
    tmp_cl_tbl<-tmp_cl_tbl %>% mutate(hub.cage.zscore=future_map(starts,function(x){
      chr_cl_cage_hic_dat %>% 
    ...
    tmp_hub_dat<-tmp_cl_tbl %>% dplyr::select(cl,hub.cage.zscore) %>% unnest(cols = c(hub.cage.zscore)) %>% 
      mutate(hub.io=ifelse(cl %in% tmp_hubs,"in","out"),zscore=as.numeric(zscore)) %>% 
      dplyr::select(chr,res,X1,X2,zscore,hub.io) %>% 
      distinct()
